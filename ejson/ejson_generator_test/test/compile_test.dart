...
import 'package:ejson/generator.dart';
...
void testCompile(String description, dynamic source, [dynamic matcher]) {
  ...
  matcher = matcher is String
      ? completion(
          equals(
            matcher.substring(
              _tag.firstMatch(_formatter.format(matcher))!.start, // strip out any thing before the tag
            ),
          ),
        )
      : matcher;
  ...
  test(description, () {
    generate() async {
      ...
      await testBuilder(
        getEJsonGenerator(),
        {
          'pkg|source.dart': "import 'package:ejson/ejson.dart';\n\n$source\n\nvoid main() {}",
        },
        writer: writer,
...
''',
      throwsA(isA<EJsonSourceError>().having((e) => e.message, 'message', 'Too many elements')),
    );
