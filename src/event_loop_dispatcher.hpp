...
#ifndef REALM_DART_EVENT_LOOP_DISPATCHER_HPP
#define REALM_DART_EVENT_LOOP_DISPATCHER_HPP
...
public:
    EventLoopDispatcher(std::shared_ptr<util::Scheduler> scheduler, util::UniqueFunction<void(Args...)> func)
        : m_func(std::make_shared<util::UniqueFunction<void(Args...)>>(std::move(func))), m_scheduler(scheduler)
    {
    }
...
template <typename... Args>
EventLoopDispatcher(std::shared_ptr<util::Scheduler>, void (*)(Args...)) -> EventLoopDispatcher<void(Args...)>;
...
EventLoopDispatcher(std::shared_ptr<util::Scheduler>, const T&) -> EventLoopDispatcher<Sig>;

struct DispatchFreeUserdata {
    const std::shared_ptr<util::Scheduler> m_scheduler;
    realm_free_userdata_func_t m_func;
    DispatchFreeUserdata(std::shared_ptr<util::Scheduler> scheduler, realm_free_userdata_func_t func = nullptr)
        : m_scheduler(scheduler), m_func(func)
    {
    }
    void operator()(void* ptr)
    {
        if (m_func) {
            m_scheduler->invoke([func = m_func, ptr]() {
                func(ptr);
            });
        }
    }
