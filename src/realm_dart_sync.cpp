...
RLM_API bool realm_dart_sync_before_reset_handler_callback(realm_userdata_t userdata, realm_t* realm)
{
    ...
    auto ud = reinterpret_cast<realm_dart_userdata_async_t>(userdata);
    auto unlockFunc = [&complete, &success, &cv](bool result) {
        complete = true;
        success = result;
        cv.notify_one();
    };
    ud->scheduler->invoke([ud, &realm, &cv, &m, &complete, &success, unlockFunc]() {
        ...
        (reinterpret_cast<realm_sync_before_client_reset_lock_func_t>(ud->dart_callback))(ud->handle, realm, (void*)&unlockFunc);
        lk.unlock(); 
    });
    std::unique_lock<std::mutex> lk(m);
...
{
    typedef void (*unlock_func_t)(bool);
    reinterpret_cast<unlock_func_t>(unlockFunc)(success);
}
