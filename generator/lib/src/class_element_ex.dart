...
extension ClassElementEx on ClassElement {
  ...
  RealmModelInfo? get realmInfo {
    try {
      ...
      final modelType = RealmModelType.values[modelInfo.value.getField("type")!.getField("type")!.toIntValue()!];
      final mappedFields = fields.realmInfo.toList();

      if (modelType == RealmModelType.embedded && mappedFields.any((field) => field.isPrimaryKey)) {
        ...
        throw RealmInvalidGenerationSourceError("Primary key not allowed on embedded objects",
            ...
            todo: 'Remove the @PrimaryKey annotation from the field or set the model type to a value different than RealmModelType.embedded.');
      }
