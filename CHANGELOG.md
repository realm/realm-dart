...
### Enhancements
* Added new Flexible sync API `RealmResults.subscribe()` and `RealmResults.unsubscribe()` as an easy way to create subscriptions and download data in background. Added `MutableSubscriptionSet.removeUnnamed()` for removing all the unnamed subscriptions. This API is experimental. ([#1354](https://github.com/realm/realm-dart/pull/1354))
