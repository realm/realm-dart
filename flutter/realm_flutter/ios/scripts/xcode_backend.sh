...
is set to release or run \"flutter build ios --release\", then re-run Archive from Xcode."
  ...
  flutter_framework="${project_path}/ios/.symlinks/plugins/realm_flutter/ios/Frameworks/Flutter/Flutter.framework"
  flutter_podspec="${project_path}/ios/.symlinks/plugins/realm_flutter/ios/Frameworks/Flutter/Flutter.podspec"
  ...
  RunCommand cp -R "${flutter_framework}/" "${derived_dir}/Flutter.framework"
  ...
  AssertExists "${flutter_framework}"
  AssertExists "${flutter_podspec}"
...
EmbedFlutterFrameworks() {
  ...
  # fi
  flutter_framework="${project_path}/ios/.symlinks/plugins/realm_flutter/ios/Frameworks/Flutter/Flutter.framework"
  echo "flutter_framework PATTH: ${flutter_framework}"
  ...
  RunCommand rsync -av --delete --filter "- .DS_Store/" --filter "- Headers/" --filter "- Modules/" "${flutter_framework}" "${xcode_frameworks_dir}/"
  if [[ "$ACTION" != "install" || "$ENABLE_BITCODE" == "NO" ]]; then
    ...
    RunCommand "${DT_TOOLCHAIN_DIR}"/usr/bin/bitcode_strip "${flutter_framework}/Flutter" -r -o "${xcode_frameworks_dir}/Flutter.framework/Flutter"
  fi
