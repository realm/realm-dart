...
is set to release or run \"flutter build ios --release\", then re-run Archive from Xcode."
  ...
  flutter_framework="${project_path}/ios/.symlinks/plugins/realm/ios/Frameworks/Flutter.framework"
...
EmbedFlutterFrameworks() {
  local project_path="${SOURCE_ROOT}/.."
  flutter_framework="${project_path}/ios/.symlinks/plugins/realm/ios/Frameworks"
...
# Embed App.framework from Flutter into the app (after creating the Frameworks directory
  ...
  RunCommand rsync -av --delete --filter "- .DS_Store" --filter "- Headers" --filter "- Modules" "${flutter_framework}/Flutter.framework" "${xcode_frameworks_dir}/"
  if [[ "$ACTION" != "install" || "$ENABLE_BITCODE" == "NO" ]]; then
    ...
    RunCommand "${DT_TOOLCHAIN_DIR}"/usr/bin/bitcode_strip "${flutter_framework}/Flutter.framework/Flutter" -r -o "${xcode_frameworks_dir}/Flutter.framework/Flutter"
  fi
