...
void main() {
  ...
  test('AppConfiguration can be initialized', () {
    Configuration.defaultRealmPath = path.join(
        Configuration.defaultStoragePath, Configuration.defaultRealmName);
    final defaultAppConfig = AppConfiguration('myapp');
    expect(defaultAppConfig.appId, 'myapp');
    expect(
        defaultAppConfig.baseFilePath.path, Configuration.defaultStoragePath);
    ...
    expect(defaultAppConfig.metadataPersistenceMode,
        MetadataPersistenceMode.plaintext);
  ...
  test('AppConfiguration can be created with defaults', () {
    ...
    expect(
        appConfig.metadataPersistenceMode, MetadataPersistenceMode.plaintext);
  ...
  test('AppConfiguration can be created', () {
    ...
    expect(appConfig.defaultRequestTimeout, const Duration(seconds: 2));
    expect(
        appConfig.metadataPersistenceMode, MetadataPersistenceMode.encrypted);
    expect(appConfig.maxConnectionTimeout, const Duration(minutes: 1));
    expect(appConfig.httpClient, httpClient);
  ...
  baasTest('App remove user', (configuration) async {
    final app = App(configuration);
    final user =
        await app.logIn(Credentials.emailPassword(testUsername, testPassword));
  ...
  baasTest('App log out anonymous user is marked as removed',
      (configuration) async {
    final app = App(configuration);
    final credentials = Credentials.anonymous();
    final user = await app.logIn(credentials);
  ...
  baasTest('App delete user', (configuration) async {
    ...
    final user = await loginWithRetry(
        app, Credentials.emailPassword(username, strongPassword));
    ...
    await expectLater(
        () => loginWithRetry(
            app, Credentials.emailPassword(username, strongPassword)),
        throws<AppException>("invalid username/password"));
  });

  baasTest('Call Atlas function that does not exist', (configuration) async {
    final app = App(configuration);
    final user = await app.logIn(Credentials.anonymous());
    await expectLater(user.functions.call('notExisitingFunction'),
        throws<AppException>("function not found: 'notExisitingFunction'"));
  });
  ...
  baasTest('Call Atlas function with one argument', (configuration) async {
    ...
    final dynamic response =
        await user.functions.call('userFuncOneArg', [arg1]);
    expect(response, isNotNull);
  ...
  baasTest('Call Atlas function with two arguments', (configuration) async {
    ...
    const arg2 = 'Michael';
    final dynamic response =
        await user.functions.call('userFuncTwoArgs', [arg1, arg2]);
    expect(response, isNotNull);
    final map = response as Map<String, dynamic>;
  ...
  baasTest('Call Atlas function with two arguments but pass one',
      (configuration) async {
    final app = App(configuration);
    final user = await app.logIn(Credentials.anonymous());
    const arg1 = 'Jhonatan';
    final dynamic response =
        await user.functions.call('userFuncTwoArgs', [arg1]);
    expect(response, isNotNull);
    final map = response as Map<String, dynamic>;
    expect(map['arg1'], arg1);
  ...
  baasTest('Call Atlas function with two Object arguments',
      (configuration) async {
    final app = App(configuration);
    final user = await app.logIn(Credentials.anonymous());
    final arg1 = Person("Jhonatan");
    final arg2 = Person('Michael');
    final dynamic response = await user.functions
        .call('userFuncTwoArgs', [arg1.toJson(), arg2.toJson()]);
    expect(response, isNotNull);
    final map = response as Map<String, dynamic>;
    final receivedPerson1 =
        PersonExt.fromJson(map['arg1'] as Map<String, dynamic>);
    final receivedPerson2 =
        PersonExt.fromJson(map['arg2'] as Map<String, dynamic>);
  ...
  baasTest('App switch to logout user throws', (configuration) async {
    ...
    expect(
      ...
      throws<RealmException>(
          "Switch user failed. User is no longer valid or is logged out"),
    );
  });
  ...
  test('bundleId is salted, hashed and encoded', () {
    ...
    final expected =
        base64Encode(sha256.convert([...salt, ...utf8.encode(text)]).bytes);
  ...
  baasTest('App(AppConfiguration) on background isolate logs warning',
      (appConfig) async {
